...
import { generateRandomNumber, delay } from '../utils/mod.js'
import {
  getContact,
  getRoom,
} from '../plugins/mod.js'
...
// ChatFlowCore.logger.info(JSON.stringify(groupNoticeData))
...
export class GroupNoticeChat {
  ...
  static async getGroupNotifications () {
    ...
    try {
      ...
      ChatFlowCore.logger.info('待发送的群发通知列表（添加接收人）：\n', JSON.stringify(groupNotifications))

      return groupNotifications
    } catch (e) {
      ChatFlowCore.logger.error('获取群发通知列表失败：\n', e)
  ...
  static async pubGroupNotifications () {
    ...
    for (const notice of groupNotifications) {
      ...
      // ChatFlowCore.logger.info('当前消息：', JSON.stringify(notice))
      if (notice.type === 'room') {
        ...
        if (room) {
          try {
            ...
            ChatFlowCore.logger.info('发送成功：群-', await room.topic())
          } catch (e) {
            ...
            ChatFlowCore.logger.error('发送失败：', e)
          }
        } else {
          notice.state = '发送失败|fail'
          notice.pubTime = timestamp
          ...
          resPub.push(notice)
          failRoom.push(notice)
          ChatFlowCore.logger.error('发送失败：群不存在', JSON.stringify(notice))
        }
      } else {
        ...
        if (contact) {
          ...
          resPub.push(notice)
          failContact.push(notice)
          ChatFlowCore.logger.error('发送失败：好友不存在', JSON.stringify(notice))
        }
      }
    }
    for (let i = 0; i < resPub.length; i = i + 10) {
      ...
      ChatFlowCore.logger.info('群发消息同步中...', i + 10)
