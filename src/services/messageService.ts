...
import { getCurTime, delay } from '../utils/utils.js'
...
export class MessageChat {
  ...
  static MEDIA_PATH_QRCODE:string
  static MEDIA_PATH_CONTACT:string
  static MEDIA_PATH_ROOM:string
  ...
  static async init () {
    ...
    try {
      ...
      setInterval(() => {
        // ChatFlowCore.logger.info('待处理消息池长度：', that.msgStore.length || '0')
        ...
        if (that.msgStore.length && ChatFlowCore.isReady) {
          ...
          // ChatFlowCore.logger.info('写入vika的消息：', JSON.stringify(records))
          try {
            ...
            ChatFlowCore.logger.error('调用datasheet.records.create失败：', err)
          }
  ...
  static addRecord (record: any) {
    ChatFlowCore.logger.info('addRecord消息入列:', JSON.stringify(record))
    if (record.fields) {
      MessageChat.msgStore.push(record)
      // ChatFlowCore.logger.info('最新消息池长度：', this.msgStore.length)
    }
  }
  ...
  static async addChatRecord (record:any) {
    ...
    try {
      ...
      ChatFlowCore.logger.error('addChatRecord添加记录失败：', e)
    }
  ...
  static async uploadQRCodeToVika (qrcode: string, status: ScanStatus) {
    ...
    if (status === ScanStatus.Waiting || status === ScanStatus.Timeout) {
      ...
      // ChatFlowCore.logger.info('StarterBot', 'vika onScan: %s(%s) - %s', ScanStatus[status], status, qrcodeImageUrl)
      ...
      try {
        ...
        filePath = `${MessageChat.MEDIA_PATH_QRCODE}/` + file.name
        try {
          ...
          if (uploadedAttachments.data) {
            try {
              ...
              //   ChatFlowCore.logger.info('二维码上传vika完成删除文件：', filePath, err)
              // })
            } catch (err) {
              ChatFlowCore.logger.error('二维码vika 写入失败：', err)
            }
          }
        } catch {
          ChatFlowCore.logger.info('二维码上传失败：', filePath)
          // fs.unlink(filePath, (err) => {
          //   ChatFlowCore.logger.info('二维码上传vika失败删除文件', filePath, err)
          // })
        }

      } catch (e) {
        ChatFlowCore.logger.error('onScan,二维码vika 写入失败：', e)
      }

    } else {
      ChatFlowCore.logger.info('机器人启动，二维码上传维格表', 'onScan: %s(%s)', ScanStatus[status], status)
    }
  }
  ...
  static async addScanRecord (uploadedAttachments: any, text: string) {
    ...
    // ChatFlowCore.logger.info('待写入登录二维码消息:', JSON.stringify(records))
    const response:any = await ServeCreateTalkRecords({ records })
    log.info('调用vika写入接口成功，写入vika结果：', JSON.stringify(response))

    // if (!response.message.success) {
    //   ChatFlowCore.logger.error('调用vika写入接口成功，写入vika失败：', JSON.stringify(response))
    // }
  ...
  static async handleFileMessage (file:FileBox, message: Message) {
    ...
    let filePath = `${MessageChat.MEDIA_PATH_CONTACT}/${talker.id}_${fileName}`
    if (room) {
      filePath = `${MessageChat.MEDIA_PATH_ROOM}/${room.id}_${fileName}`
    }

    // ChatFlowCore.logger.info('文件路径filePath:', filePath)

    try {
      await file.toFile(filePath, true)
      ChatFlowCore.logger.info('保存文件到本地成功', filePath)
    } catch (err) {
      ChatFlowCore.logger.error('保存文件到本地失败', err)
      return ''
    }
  ...
  static async addHeartbeatRecord (text: string) {
    ...
    ChatFlowCore.logger.info('心跳消息:', JSON.stringify(record))
    MessageChat.msgStore.push(record)
  }
  ...
  static async onMessage (message: Message) {
    ...
    try {
      ...
      if (file) {
        // ChatFlowCore.logger.info('文件file:', file)
        try {
      ...
      try {
        ...
        ChatFlowCore.logger.error('onMessage添加记录失败：', e)

      }

    } catch (e) {
      ChatFlowCore.logger.error('onMessage消息转换失败：', e)
    }
  }
