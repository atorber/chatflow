...
async function handleAutoQAForContact (message: Message, keyWord: string) {
  ...
  const AUTOQA_AUTOREPLY = ChatFlowCore.configEnv.AUTOQA_AUTOREPLY
  log.info('自动问答开关开启状态：', AUTOQA_AUTOREPLY ? '开启' : '关闭')

  // 问答开关开启，私聊中不需要@机器人
  if (AUTOQA_AUTOREPLY) {
    // 判断是否在微信对话平台白名单内
    const isInContactWhiteList = await containsContact(ChatFlowCore.whiteList.contactWhiteList.qa, talker)
    if (isInContactWhiteList) {
      log.info('当前好友在【白名单|WhiteList/智能问答|qa】内，请求问答...')
      try {
        await wxai(ChatFlowCore.configEnv, ChatFlowCore.bot, message)
      } catch (e) {
    ...
    // 判断是否在gpt白名单内
    const isInGptContactWhiteList = await containsContact(ChatFlowCore.whiteList.contactWhiteList.gpt, talker)
    if (isInGptContactWhiteList) {
      log.info('当前好友在【白名单|WhiteList/ChatGPT|gpt】内，请求问答gpt...')
      try {
        await gpt(ChatFlowCore.bot, message)
      } catch (e) {
...
async function handleAutoQA (message: Message, keyWord: string) {
  ...
  if (AUTOQA_AUTOREPLY && includesKeyWord) {
    ...
    // 判断是否在微信对话平台白名单内
    const isInRoomWhiteList = await containsRoom(ChatFlowCore.whiteList.roomWhiteList.qa, room)
    ...
    // 判断是否在gpt白名单内
    const isInGptRoomWhiteList = await containsRoom(ChatFlowCore.whiteList.roomWhiteList.gpt, room)
