...
export class ChatFlowConfig {
  ...
  static async init (config:ConfigTypes) {
    ...
    if (this.spaceId) {
        ...
        if (sheet && !tables[sheet.name]) {
          ...
          for (let j = 0; j < fields.length; j++) {
            ...
            switch (field?.type) {
              ...
              // case 'MagicLink':
              //   newField.property = {}
              //   newField.property.foreignDatasheetId = this[field.desc as keyof ChatFlowConfig]
              //   if (field.desc) {
              //     newFields.push(newField)
              //   }
              //   break
      ...
      let configVika: any[] = await this.getAllRecords(this.dataBaseIds.envSheet)
      configVika = configVika.filter((item: any) => item.fields['标识|key'] === 'WECHATY_PUPPET' || item.fields['标识|key'] === 'WECHATY_TOKEN')

      const wechatyConfig: WechatyConfig = {
        puppet: configVika[0].fields['值|value'],
        token: configVika[1].fields['值|value'],
      }
  ...
  protected static async getAllRecords (datasheetId: string) {
    let records: any = []
    const datasheet = await this.vika.datasheet(datasheetId)
